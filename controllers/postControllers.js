...
const createPost = async (req, res, next) => {
    try {
        ...
        if(thumbnail.size > 2000000){
            return next(new HttpError("Thumbnail to big. File should be less than 2MB"))
        }
        ...
        fs.unlink(path.join(__dirname, '..', 'uploads', fileName), async (err) => {
            if(err){
                ...
                if(currentUser?.post <= 0){
                    const UserPostCount = currentUser?.post
                    await User.findByIdAndUpdate(req.user.id, {post: UserPostCount})
                }
                const UserPostCountDec = currentUser?.post - 1
                await User.findByIdAndUpdate(req.user.id, {post: UserPostCountDec})
            }
        })
